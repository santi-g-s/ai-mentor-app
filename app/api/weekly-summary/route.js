...
export async function GET() {
  try {
    ...
    console.log(`Fetching sessions from ${oneWeekAgo.toISOString()} to ${new Date().toISOString()}`);
    ...
    let summary = "Summary generation skipped. To enable AI-powered summaries, add GOODFIRE_API_KEY to your environment.";
    ...
    if (process.env.GOODFIRE_API_KEY) {
      try {
        ...
        const summaryResponse = await generateWeeklySummary(jsonSessions, openai);
        
        // Handle the response safely - the model is returning plain text, not JSON
        try {
          // Try to parse as JSON first
          const summaryObj = JSON.parse(summaryResponse);
          summary = summaryObj.weekSummary || summaryResponse;
        } catch (parseError) {
      ...
      console.log("GOODFIRE_API_KEY not found in environment. Skipping summary generation.");
    }
  ...
  DO NOT include any explanations, introductions, or additional formatting. ONLY return the JSON object.`;
      ...
      const response = await openai.chat.completions.create({
        ...
        messages: [
          ...
          { role: "user", content: `Here are my therapy/mentoring session transcripts from the past week. Please provide a 2-3 sentence summary of these sessions as a whole:\n\n${transcriptsText}` }
        ],
        max_tokens: 300,
        temperature: 0.5,
      });
